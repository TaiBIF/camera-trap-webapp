<template>
  <div class="container calibox">
    <router-link
      :to="`/project/${$route.params.id}/site/${$route.params.site_id}`"
      class="text-muted"
    >
              ...
              <div class='css_tr'>
                <div class='css_td'>
                  <div class='ctrlZone'>
                    ...
                    <div class="row">
                      ...
                      <div class="col-6 text-right">
                        <button
                          class='main cancel-editing-token-button btn btn-default btn-sm'
                          @click='cancelEditingToken($event)'
                          v-if='edit_token'
                        >
                          取消選取
                        </button>
                  ...
                  >
                    ...
                    <div
                      class='outer left'
                      v-if='tokenCreating||!edit_token==false'
                      aria-hidden="true"
                    ></div>
                    <div
                      class='outer top'
                    ...
                    <div
                      id='edit-bbox'
                      :class="{'show': !edit_token==false}"
                    ></div>
                    <img
                      id='target_image'
                      :src='ann.url'
                      imageonload='img2data()'
                    />
                    <!-- <br/> -->
          ...
          <div class="col-5 info-container">
            ...
            <div class='css_td gray-block table-container'>
              <div class="tab-header">
                <a
                  :class="tableIndex === 0 ? 'active' : ''"
                  @click="tableIndex=0"
                >進階標註</a>
                <a
                  :class="tableIndex === 1 ? 'active' : ''"
                  @click="tableIndex=1"
                >關聯標註</a>
                <a
                  :class="tableIndex === 2 ? 'active' : ''"
                  @click="tableIndex=2"
                >標註範圍資料</a>
              </div>
                  ...
                  >
                    <div label="Data">
                      <table v-if="!edit_token === false && edit_token.data.length">
                        <tr
                          v-for='(data,di) in edit_token.data'
                          :key="`data-${di}`"
                        >
                          <td>
                            <input
                              name='data-label'
                              placeholder="Label"
                              class='user-input user-input-data data-label user-input-first-cell'
                              :nrow='`data-${di}`'
                              :data-row="di"
                            >
                          </td>
                          <td>
                            <input
                              ...
                              :nrow='`data-${di}`'
                              :data-row="di"
                              class='`user-input user-input-data data-key`'
                              v-model='edit_token.data[di].key'
                              placeholder="Key"
                            >
                          </td>
                          <td>
                            ...
                            >
                            <div
                              :data-key="edit_token.data[di].key"
                              v-if='showSelectionBox[edit_token.token_id + "_" + edit_token.data[di].key + "-di-" + di]'
                            >
                              <span
                                class='elm_searh'
                          ...
                          <td>
                            <input
                              ...
                              @focus='addDataEntry(di === edit_token.data.length-1)'
                            ></td>
                        </tr>
                      </table>
                      <div class="col-12 mt-2 mb-3">
                        <button
                          class='btn btn-sm btn-default add-a-row-button add-data mr-2'
                        ...
                        >Save</button>
                        <button
                          class='btn btn-sm btn-default cancel-editing-token-button'
                          @click='cancelEditingToken($event)'
                          v-if='edit_token'
                        >cancel editing token</button>
                      </div>
                    </div>
                ...
                <form v-if='edit_token && !showSelectionBox==false'>
                  <div label="Relations">
                    <table v-if="!edit_token === false && !edit_token.relations === false">
                      ...
                      <tr
                        v-for='(rel, ri) in edit_token.relations'
                        :key="`tr-itemn-${ri}`"
                      >
                        <td>
                          <input
                            ...
                            :class="`relation-row-${ri}`"
                            class='user-input user-input-relation relation-context user-input-first-cell'
                            v-model='edit_token.relations[ri].context'
                            placeholder="Context"
                          >
                        </td>
                        <td>
                          <input
                            ...
                            :nrow='`relation-${ri}`'
                            :class='`relation-row-${ri}`'
                            class='user-input user-input-relation relation-relation'
                            v-model='edit_token.relations[ri].relation'
                        ...
                        <td>
                          <input
                            ...
                            :nrow='`relation-${ri}`'
                            class='user-input user-input-relation relation-object'
                            @keydown="ctrlPasteCopiedToken($event, ri)"
                            v-model='edit_token.relations[ri].object'
                            placeholder="Object"
                            @focus="readyToRelate($event, ri); addRelEntry(ri === edit_token.relations.length-1)"
                            @blur="cancelToRelate($event)"
                            :class="{'last-relation-cell' : (di === edit_token.data.length-1)}"
                          >
                        </td>
                        <td>
                          <div
                            v-if="!rel.object === false"
                            @click="resolveUrlToken($event, rel.object)"
                          >
                            resolve
                          </div>
                        </td>
                        <td>
                          <button
                            v-if="isReadyToRelate"
                            @mousedown="tokenIDFromCopiedToken($event, ri)"
                          >
                            Paste Copied Token
                          </button>
                        </td>
                    ...
                    <div class="col-12 mt-2 mb-3">
                      <button
                        class='btn btn-sm btn-default add-a-row-button add-relation mr-2'
                        @click='addRelEntry(true)'
                      >Add a row</button>
                      <button
                        class="btn btn-sm btn-default mr-2"
                        @click='updateToken()'
                      ...
                      <button
                        ...
                        @click='cancelEditingToken($event)'
                        v-if='edit_token'
                      >cancel editing token</button><br />
                    </div>
                  </div>
                </form>
              </div>
                ...
                >
                  ...
                  <div class='row'>
                    ...
                    <div class='col-8'>
                      <input
                        name='viid'
                        v-model='edit_token.meta.virtual_individual_id'
                        @keydown="ctrlPasteSth($event)"
                      >
                    </div>
                    <div class='col-8 offset-4 mt-1 mb-2'>
                      <button
                        class="btn btn-sm btn-default mr-2"
                        @click='edit_token.meta.virtual_individual_id = uuid();'
                      >new</button>
                      <button
                        class="btn btn-sm btn-default"
                        @click='simpleSearch({viid: edit_token.meta.virtual_individual_id})'
                      >look for me</button>
                    </div>
                  </div>
                  <div class='row'>
                    ...
                    <div class='col-8'>
                      <input
                        name='vpid'
                        v-model='edit_token.meta.virtual_part_id'
                        @keydown="ctrlPasteSth($event)"
                      >
                    </div>
                    <div class='col-8 offset-4 mt-1 mb-2'>
                      <button
                        class="btn btn-sm btn-default"
                        @click='edit_token.meta.virtual_part_id = uuid();'
                      >new</button>
                    </div>
                  </div>
                </div>
...
class Token {
  ...
  cloneData(data) {
    ...
    var clonedData = [];
    data.forEach(d => {
      var clonedD = window.$.extend(true, {}, d);
      clonedData.push(clonedD);
    });
  ...
  cloneRel(rel) {
    ...
    var clonedRel = [];
    rel.forEach(r => {
      var clonedR = window.$.extend(true, {}, r);
      clonedR.object = this.obj._makeUrlToken(null, clonedR.object);
      clonedRel.push(clonedR);
    });
    this.relations = clonedRel;
  }
  ...
  removeEmptyRelEntry() {
    ...
    this.relations = this.relations.filter(el => {
      if (
        !el.relation.toString().trim() === false &&
        !el.object.toString().trim() === false
      ) {
        return true;
      } else {
        return false;
...
export default {
  ...
  data() {
    return {
      ...
      image_list: [],
      hasMergedSessionImageList:
        !window.localStorage.getItem('mergedList') === false,
      max_retry: 5,
      retry: 0,
  ...
  methods: {
    AnnImg() {
      ...
      const iframeWins = document.getElementsByTagName('iframe');
      iframeWins.forEach(frm => {
        try {
          const frmImgs = frm.contentWindow.document.getElementsByTagName(
            'img',
          );
          if (frmImgs.length > 0) {
            frmImgs.forEach(frmImg => {
              if (frmImg.width >= 300 && frmImg.height >= 300) {
                images.push(frmImg.src);
              }
            });
          }
        } catch (err) {
      ...
      const flickrBgimgDivs = document.getElementsByClassName(
        'photo-list-photo-view',
      );
      flickrBgimgDivs.forEach((item, d) => {
        var csstxt = 'div { ' + flickrBgimgDivs[d].getAttribute('style') + '}';
        var css = rulesForCssText(csstxt);
        var bgImgMatch = css[0].style.backgroundImage.match(
          /^url\(&quot;(.+)&quot;\)$/,
        );
        if (!bgImgMatch === false) {
          var bgImg = bgImgMatch[1].match(/^http/)
            ? bgImgMatch[1]
            : window.location.protocol + bgImgMatch[1];
          images.push(bgImg);
        }
      });

      var carrierUrl = window.location.href;
      ...
      var annUrl =
        'http://twebi.net/imgAnn/ann.html?bookmarklet=true&amp;images_string=' +
        encodeURIComponent(imagesString) +
        '&amp;carrier_url=' +
        encodeURIComponent(carrierUrl) +
        '&amp;title=' +
        title;

      window.open(annUrl, 'annImg');
    },
    uiCopyToken(e) {
      e.stopPropagation();
    ...
    uiPasteToken(e, asNew, dataOnly) {
      ...
      if (!window.$.isEmptyObject(this.uiCopiedToken)) {
        ...
        var editTokenId = this.edit_token.token_id;
        ...
        this.edit_token.token_id = editTokenId;
        this.styleEditBox(this.edit_token.getBBox());
    ...
    uiDeleteToken(e, tid) {
      ...
      this.ann.tokens.splice(whichToken, 1);
      var token = null;
      this.tree = { tokens: [] };

      this.ann.tokens.forEach(tok => {
        token = tok;
        token.tokens = [];
      });

      this.ann.tokens.forEach(token => {
    ...
    setRelationObject(e, tokenId) {
      e.preventDefault();
      this.edit_token.relations[this.tokenRelationIdx].object = tokenId;
    },
    formatJSON(obj) {
    ...
    switchImage(direction, e) {
      ...
      var lastImageIdx = this.current_image_idx;
      ...
      if (lastImageIdx !== this.current_image_idx) {
    ...
    updateToken() {
      ...
      this.bbox[et.token_id] = newBox;
      this.meta[et.token_id] = et.meta;
      this.boxStyle[et.token_id] = this.styleMyBox(newBox);
      ...
      if (!tokenExists) {
        ...
        this.ann.tokens[whichToken].cloneToken(et);
      }
    ...
    _makeUrlToken(url, tokenId) {
      if (!tokenId) return '';

      var frags = [];

      var test = this._getUrlToken(tokenId);
      tokenId = test.token_id;
    ...
    setEditToken(tid, e, skipE) {
      ...
      if (!skipE) {
        ...
        if (window.$(e.target).hasClass('bbox') > -1) {
          $eTarget = window.$('.' + tid + '.token-basic')[0];
        } else if (window.$(e.target).hasClass('token-basic') > -1) {
          $eTarget = e.target;
        } else {
          $eTarget = window.$(e.target).parents('.token-basic')[0];
        }

        window.$('.token-basic').removeClass('focus');
        window.$($eTarget).addClass('focus');

        $eTarget.classList.forEach(function(c) {
          var m = c.match(/^tree-simple-id-(.+)$/);
          if (!m === false && !m[1] === false) {
      ...
      const $targetImage = window.$('#target_image');
      const px = e.pageX - $targetImage.offset().left;
      const py = e.pageY - $targetImage.offset().top;
      const startX = this.edit_bbox.start_x;
      const startY = this.edit_bbox.start_y;
      const x = Math.min(px, startX);
      const y = Math.min(py, startY);
      const maxX = Math.max(px, startX);
      const maxY = Math.max(py, startY);
      const w = maxX - x;
      const h = maxY - y;
    ...
    _getImageData(url) {
      ...
      if (!this.image_url === false) {
        if (window.location.port === '8888') {
          ...
          var queryId = this.current_query_id;
          this.current_query_id += 1;
          this.$http
            .post(
              '/annotation/mongo_load.php',
              { url: this.image_url, qid: queryId },
              true,
            )
            .then(ret => {
              console.log(ret);
              if (!ret.result || ret.result === 'null') {
                ret = [];
                var jsonFile = '/annotation/annotation.empty.json';
                ret.forEach(e => {
                  if (e.image_url === this.image_url) jsonFile = e.json_file;
                });

                this.$http.get(jsonFile, {}).then(ret => {
    ...
    styleMyBox(box) {
      ...
      if (box.full_scope) {
        const $targetImage = document.querySelector('#target_image');
        const imgH = $targetImage.clientHeight;
        const imgW = $targetImage.clientWidth;
        boxStyle.width = imgW;
        boxStyle.height = imgH;
      } else {
    ...
    startAnnotation(ret) {
      if (!this.auto_copy_wall_tokens || ret.tokens.length > 0) {
        ...
        this.ann.tokens.forEach(t => {
          var tmp = new Token(this);
          tmp.create();
          var tmpTokenId = tmp.token_id;
          old2New[t.token_id] = tmpTokenId;
          tmp.cloneToken(t);
          tmp.token_id = tmpTokenId;
          ret.tokens.push(tmp);
        });

        ret.tokens.forEach(t => {
          if (!t.meta === false && !t.meta.parent_id === false) {
      ...
      this.ann.url = annJson.url;
      var tmpCarrierUrl = annJson.carrier_url;
      this.ann.carrier_url = tmpCarrierUrl;
      this.carrier_url = tmpCarrierUrl;
      ...
      if (!annJson.tokens === false) {
        annJson.tokens.forEach((t, i) => {
          var token = new Token(this);
          token.cloneToken(annJson.tokens[i]);
          ...
          var colorName = this.color_names[i % this.color_names.length];

          var box = this.bbox[token.token_id];

          var boxStyle = this.styleMyBox(box, colorName);
      ...
      annJson.tokens = tokens;
      annJson.relations = undefined;
    ...
    saveAnnotation(_callback) {
      ...
      if (!window.$.isEmptyObject(this.edit_token)) {
        editTokenId = this.edit_token.token_id;
      }
      this.rebuildTree();
      ...
      this.$http
        ...
        .then(function(ret) {
          ...
          // this.edit_token_form.$setPristine()
          if (editTokenId) {
            this.setEditToken(editTokenId, null, true);
          }
    ...
    imgmm(e) {
      if (
        ...
        const $targetImage = window.$('#target_image');
        var px = e.pageX - $targetImage.offset().left;
        var py = e.pageY - $targetImage.offset().top;
        var startX = this.edit_bbox.start_x;
        var startY = this.edit_bbox.start_y;
        var x = Math.min(px, startX);
        var y = Math.min(py, startY);
        var maxX = Math.max(px, startX);
        var maxY = Math.max(py, startY);
        var w = maxX - x;
        var h = maxY - y;
    ...
    imgmu(e) {
      if (this.tokenCreating && this.init_edit_box) {
        this.init_edit_box = false;
        const $targetImage = window.$('#target_image');
        var endX = e.pageX - $targetImage.offset().left;
        var endY = e.pageY - $targetImage.offset().top;
        ...
        var newBbox = {
          ...
          original_point: 'topleft',
          x: {
            min: Math.min(this.edit_bbox.start_x, endX),
            max: Math.max(this.edit_bbox.start_x, endX),
          },
          y: {
            min: Math.min(this.edit_bbox.start_y, endY),
            max: Math.max(this.edit_bbox.start_y, endY),
          },
        };

        newToken.cloneBBox(newBbox);

        this.edit_token = newToken;
        this.styleEditBox(newBbox);
    ...
    _rebuild(
      ...
      if (!token.meta === false) {
        token.meta.parent_id = parentToken.token_id;
      }
      ...
      var revertViid = true;
      if (!parentToken === false) {
        if (
          (!parentToken.isGroup() && parentToken.meta.virtual_individual_id) ||
          inheritIdv
        ) {
          inheritIdv = true;
          token.meta.virtual_individual_id =
            parentToken.meta.virtual_individual_id;
          revertViid = false;
        }
      }
      ...
      if (revertViid && !token.meta === false && !token.isGroup()) {
        token.meta.virtual_individual_id =
          token.meta.original_virtual_individual_id;
      }
      ...
      if (token.description_level === 'individual') {
        ...
        if (!token.meta === false && !token.isGroup()) {
      ...
      if (!token.token_id === false) {
        parentToken = token;
        this.boxStyle[token.token_id]['z-index'] = zIndex;
        ...
        if (token.token_id === this.try_to_focus_me) {
          ...
          const skipE = true;
          this.setEditToken(token.token_id, null, skipE);
        ...
        parentToken = new Token(this);
        parentToken.cloneToken(tplToken);
      }
      ...
      if (!token.token_id === false) {
        ...
        if (
          !token.meta === false &&
          !token.meta.virtual_individual_id === false
        ) {
    ...
    tokenIDFromCopiedToken(e, ri) {
      e.preventDefault();
      var urlTokenId = window.localStorage.getItem('ui_copied_token_id');
      var tokenIdParts = urlTokenId.split('#');
      var tokenId = tokenIdParts.pop();

      if (tokenIdParts.join('') === this.ann.url) {
        this.edit_token.relations[ri].object = tokenId;
      } else {
        this.edit_token.relations[ri].object = urlTokenId;
      }
    },
    ...
    focusTokenBasic() {
      if (!window.$.isEmptyObject(this.edit_token)) {
        var focusedTokenId = this.edit_token.token_id;
        if (!window.$.isEmptyObject(this.classes[focusedTokenId])) {
          this.classes[focusedTokenId]['focus'] = true;
        }
      }
    },
    autoValueClicked(di, v) {
      this.edit_token.data[di].value = v.vname
        .replace(/\(.+\)/g, '')
        .replace(/ +/, ' ')
        .trim();
      this.showSelectionBox[
        this.edit_token.token_id +
          '_' +
          this.edit_token.data[di].key +
          '-di-' +
          di
      ] = false;
    ...
    autoValues(di) {
      ...
      if (window.$.isEmptyObject(this.autoRes[attrName])) {
        this.$http.get('/annotation/splist.csv', {}).then(function(ret) {
          var res = [];
          var lines = ret.split('\n');
    ...
    resolveUrlToken(e, urlToken) {
      ...
      var url = ut.url;
      var tokenId = ut.token_id;

      this.try_to_focus_me = tokenId;
  ...
  mounted() {
    ...
    this.$el.querySelector('#carrier_frame').onload = () => {
      const $targetImage = this.$el.querySelector('#target_image');
      console.log('onload frame');
      if (
        !$targetImage ||
        ($targetImage.naturalHeight === 0 || !$targetImage.complete)
      ) {
      ...
      } else if ($targetImage.naturalHeight !== 0 && $targetImage.complete) {
        this.hasSolidImageList = true;
      }
    ...
    window.$(document).keydown(function(e) {
      ...
      if (
        ...
        e.target.getAttribute('data-no-text') !== null
      ) {
        if (e.key === 'g') {
          if (!window.lastG) {
          ...
          var timeDiff = Date.now() - window.lastG;
          window.lastG = Date.now();
          if (timeDiff <= 300) {
          ...
          if (!window.tokenCreating) {
            e.preventDefault();
            window._focusOnTokenBasicBySimpleID(
              window._nextToken(window.focusedTokenSimpleID),
              true,
            );
          }
          ...
          if (!window.tokenCreating) {
            e.preventDefault();
            window._focusOnTokenBasicBySimpleID(
              window._prevToken(window.focusedTokenSimpleID),
              true,
            );
          }
          ...
          if (
            !(
              window.annScope.tokenCreating &&
              !window.annScope.edit_token &&
              !window.showAnnCtrl
            )
          ) {
            window.$('#tree_and_form_toggle').click();
          ...
          if (e.ctrlKey) {
            ...
            if (window.$.isEmptyObject(window.annScope.edit_token)) {
              if (
                ...
                !window.annScope.auto_select_full_scope_on_create
              ) {
                window.$('#tree_and_form_toggle').click();
              }
          ...
          if (
            window.$('#create-new-token-button').attr('disabled') ===
              'disabled' ||
            window.$('.main.cancel-editing-token-button').css('display') !==
              'none'
          ) {
          ...
          if (!window.escapeTarget === false) {
            ...
            if (
              window.$(window.escapeTarget).parents('md-tab-content').length >
                0 &&
              window.$(window.escapeTarget).parents('.md-active').length > 0
            ) {
              window.$(window.escapeTarget).focus();
            } else {
              window
                .$('.md-active .user-input')
                .first()
                .focus();
            }
          ...
          e.preventDefault();
          window
            .$('.md-active .user-input-first-cell')
            .last()
            .focus();
        } else if (
          e.keyCode === 119 ||
        ...
        if (e.key === 'ArrowDown') {
          ...
          var cellId =
            ...
            '-di-' +
            di;
          if ($this.showSelectionBox[cellId]) {
            $this.showSelectionBox[cellId] = false;
          } else {
            // $(e.target).blur()
            window.$('.token-basic-tree').focus();
          }
    ...
    window.$('#tree_and_form').dblclick(function(e) {
      if (
        ['TD', 'INPUT', 'CANVAS', 'BUTTON'].indexOf(e.target.tagName) === -1
      ) {
        window.$('#tree_and_form_toggle').click();
      }
    });
    window.$('#tree_and_form_toggle').click(function() {
      if (window.showAnnCtrl) {
        ...
        window
          .$(this)
          .parents('.ctrl_panel')
          .addClass('annCtrlPanelHide');
        window
          .$(this)
          .parents('.ctrl_panel')
          .removeClass('annCtrlPanelShow');
      } else {
        window
          .$(this)
          .parent()
        ...
        window
          .$(this)
          .parents('.ctrl_panel')
          .addClass('annCtrlPanelShow');
        window
          .$(this)
          .parents('.ctrl_panel')
          .removeClass('annCtrlPanelHide');
        window.$('.token-basic-tree').focus();
      }
