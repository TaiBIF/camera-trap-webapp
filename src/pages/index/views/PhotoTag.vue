                    ...
                    <table v-if="!edit_token === false && !edit_token.relations === false">
                      ...
                      <tr v-for='(rel, ri) in edit_token.relations' :key="`tr-itemn-${ri}`">
                        ...
                        <td>
                          ...
                          placeholder="Object"
                          @focus="readyToRelate($event, ri); addRelEntry(ri === edit_token.relations.length-1)" @blur="cancelToRelate($event)"
...
export default {
  ...
  data () {
    return {
      ...
      current_query_id: 0,
      color_names: [
        'navy', 'blue', 'aqua', 'teal', 'olive', 'green', 'lime', 'yellow', 'orange', 'fuchsia', 'purple', 'maroon', 'gray', 'silver', 'black'
      ],
      image_list: [],
      hasMergedSessionImageList: !window.localStorage.getItem('mergedList') === false,
      max_retry: 5,
      retry: 0,
  ...
  methods: {
    AnnImg () {
      ...
      iframeWins.forEach((frm, j) => {
        try {
          const frmImgs = frm.contentWindow.document.getElementsByTagName('img')
          if (frmImgs.length > 0) {
            frmImgs.forEach((frmImg, k) => {
      ...
      const flickrBgimgDivs = document.getElementsByClassName('photo-list-photo-view')
      ...
      var annUrl = 'http://twebi.net/imgAnn/ann.html?bookmarklet=true&amp;images_string=' + encodeURIComponent(imagesString) + '&amp;carrier_url=' + encodeURIComponent(carrierUrl) + '&amp;title=' + title
    ...
    uiCopyToken (e) {
      ...
      if (!window.$.isEmptyObject(this.edit_token)) {
        ...
        this.uiCopiedToken.token_id = this.ann.url + '#' + this.edit_token.token_id
      }
    ...
    uiPasteToken (e, asNew, dataOnly) {
      ...
      this.uiCopiedToken = JSON.parse(window.localStorage.getItem('ui_copied_token'))
    ...
    uiDeleteToken (e, tid) {
      ...
      if (
        !window.$.isEmptyObject(this.edit_token) && this.edit_token.token_id === tid
      ) {
      ...
      this.ann.tokens.forEach((tok, i) => {
        token = tok
        token.tokens = []
      })

      this.ann.tokens.forEach((token, j) => {
    ...
    _getImageList () {
      if (
        ...
        if (this.image_list.length > 0) {
          ...
          document.querySelector('#carrier_frame').setAttribute('src', this.carrier_url)
          this._selectImage(0)
        }
      } else {
    ...
    _getImageData (url) {
      ...
      if (!this.image_url === false) {
        if (window.location.port === '8888') {
          const ret = {
            url: this.image_url,
            carrier_url: !this.image2carrier[this.image_url] === false ? this.image2carrier[this.image_url] : '',
          ...
          this.$http.post('/annotation/mongo_load.php', { url: this.image_url, qid: queryId }, true)
            .then(ret => {
              console.log(ret)
              if (!ret.result || ret.result === 'null') {
                ...
                this.$http.get(jsonFile, {}).then(ret => {
                  ...
                  ret.carrier_url = !this.image2carrier[this.image_url] === false ? this.image2carrier[this.image_url] : ''
    ...
    rebuildTree () {
      ...
      this._rebuild(this.tree, null, false, 0, '1', danglingPart, nonDanglingPart, clearEditToken)
    ...
    _rebuild (token, parentToken, inheritIdv, zIndex, lvl, danglingPart, nonDanglingPart, clearEditToken) {
      zIndex++
      ...
      if (!parentToken === false) {
        if (
          (!parentToken.isGroup() && parentToken.meta.virtual_individual_id) || inheritIdv
        ) {
          inheritIdv = true
          token.meta.virtual_individual_id = parentToken.meta.virtual_individual_id
          revertViid = false
        }
      }
      ...
      if (!token.token_id === false) {
        ...
        for (var k in this.classes[token.token_id]) {
          if (
            this.classes[token.token_id].hasOwnProperty(k) && k.match(/tree-simple-id-/)
          ) {
      ...
      if (!token.token_id === false) {
        ...
        if (!token.meta === false && !token.meta.virtual_individual_id === false) {
          if (
            token.meta.virtual_individual_id === token.meta.virtual_part_id
          ) {
            if (
    ...
    ctrlPasteSth (e) {
      ...
      if (!window.$.isEmptyObject(window.uiCopiedToken)) {
        ...
        case 'viid':
          ...
          window.edit_token.meta.virtual_individual_id = window.uiCopiedToken.meta.virtual_individual_id
          break
        case 'vpid':
          // window.edit_token_form.vpid.$setDirty()
          window.edit_token.meta.virtual_part_id = window.uiCopiedToken.meta.virtual_part_id
          break
        }
      }
    },
    ...
    window.$(document).keydown(function (e) {
      ...
      if (e.target.tagName !== 'INPUT' || e.target.getAttribute('data-no-text') !== null) {
        ...
        if (e.key === 'ArrowDown') {
          ...
          var cellId = $this.edit_token.token_id + '_' + $this.edit_token.data[di].key + '-di-' + di
