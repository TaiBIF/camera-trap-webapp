...
$(function() {
  ...
  var token2descLvl = {};
  var doMeLater = [];
  var action = 'https://api-dev.camera-trap.tw/media/annotation/query';

  var all_tokens = [];

  image_urls.slice(0, 10).forEach(function(url) {
    var url = url;
    var data = { query: { url: url } };

    var _callback = function(res) {
      //console.log(5566);
      if (!!res.results && !!res.results[0].tokens)
        res.results[0].tokens.forEach(function(t) {
          ...
          if (
            !!t.meta.virtual_individual_id &&
            $cy.$('#' + t.meta.virtual_individual_id).length == 0
          ) {
            $cy.add({
              group: 'nodes',
              classes: 'individual',
              data: { id: t.meta.virtual_individual_id },
              position: { x: 0, y: 0 },
            });
          }

          if (
            !!t.meta.virtual_part_id &&
            $cy.$('#' + t.meta.virtual_part_id).length == 0
          ) {
            $cy.add({
              group: 'nodes',
              classes: 'square',
              data: {
                ...
                desc_lvl: t.description_level,
                id: t.meta.virtual_part_id,
              },
              position: { x: 0, y: 0 },
            });
          }
          ...
          if (
            ...
            for (var di = 0; di < t.data.length; di++) {
              if (t.data[di].key == 'species' && !!t.data[di].value) {
                idv_label = t.data[di].value;
              } else if (
                t.data[di].key == 'category-zhtw' &&
                !!t.data[di].value
              ) {
              ...
              if (
                ...
                if (
                  ...
                  $cy.add({
                    ...
                    data: {
                      id:
                        t.meta.virtual_individual_id + '_label_' + label_as_id,
                      source: t.meta.virtual_individual_id,
                      target: label_as_id,
                      label: 'label',
                    },
                  });
          ...
          var token_parent = t.meta.virtual_part_id
            ? t.meta.virtual_part_id
            : t.meta.virtual_individual_id
              ? t.meta.virtual_individual_id
              : undefined;
          ...
          if ($cy.$('#' + t.token_id).length == 0) {
            if (t.token_id) {
              ...
              all_tokens.push(t);
              getsrc({ query: { 'tokens.token_id': t.token_id } });
            }
          }
          ...
          if (!t.meta.virtual_individual_id) {
            ...
            if (idv_label) {
              var label_as_id = simpleID(idv_label); //encodeURIComponent(idv_label).replace(/%/g, '_');
              if (
                $cy.$('#' + t.token_id).length > 0 &&
                $cy.$('#' + label_as_id).length > 0
              ) {
                if (
                  $cy.$('#' + t.token_id + '_label_' + label_as_id).length == 0
                ) {
                  $cy.add({
                    group: 'edges',
                    data: {
                      id: t.token_id + '_label_' + label_as_id,
                      source: t.token_id,
                      target: label_as_id,
                      label: 'label',
                    },
      ...
      if (!!res.results && !!res.results[0].tokens)
        res.results[0].tokens.forEach(function(t) {
          if (!!t.token_id && !!t.meta.parent_id) {
            if (
              $cy.$('#' + t.token_id).length > 0 &&
              $cy.$('#' + t.meta.parent_id).length > 0
            ) {
              var rel = 'lower_token';
              if (
                $cy.$('#' + t.token_id + '_' + rel + '_' + t.meta.parent_id)
                  .length == 0
              ) {
          ...
          if (
            !!t.meta.virtual_part_id &&
            !!t.meta.virtual_individual_id &&
            t.meta.virtual_part_id != t.meta.virtual_individual_id &&
            $cy.$('#' + t.meta.virtual_part_id).length > 0 &&
            $cy.$('#' + t.meta.virtual_individual_id).length > 0
          ) {
            if (
              ...
              $cy.add({
                ...
                data: {
                  id:
                    ...
                    '_is_part_of_' +
                    t.meta.virtual_individual_id,
                  source: t.meta.virtual_part_id,
                  target: t.meta.virtual_individual_id,
                  label: 'is_part_of',
                },
              });
            }
          ...
          if (t.relations)
            t.relations.forEach(function(r) {
              ...
              if (!!t.token_id && !!rel_token_id) {
                if (
                  $cy.$('#' + t.token_id).length > 0 &&
                  $cy.$('#' + rel_token_id).length > 0
                ) {
                  if (
                    $cy.$(
                      '#' + t.token_id + '_' + r.relation + '_' + rel_token_id,
                    ).length == 0
                  ) {
              ...
              if (!!src && !!tgt) {
                if (
                  ...
                  needDoMeLater = false;
                  if (
                    $cy.$('#' + src + '_' + r.relation + '_' + tgt).length == 0
                  ) {
                    $cy.add({
                      group: 'edges',
                      classes: 'relation',
                      data: {
                        id: src + '_' + r.relation + '_' + tgt,
                        source: src,
                        target: tgt,
                        label: r.relation,
                      },
      ...
      if (counter == 0) {
        ...
        all_tokens.forEach(function(t) {
          ...
          if (
            !!vp_id &&
            !!t2p_id &&
            $cy.$('#' + vp_id + '_is_part_of_' + t2p_id).length == 0
          ) {
            if (
              !!vp_id &&
              !!t.meta.virtual_individual_id &&
              vp_id != t.meta.virtual_individual_id &&
              $cy.$('#' + vp_id).length > 0 &&
              $cy.$('#' + t.meta.virtual_individual_id).length > 0
            ) {
              if (
                $cy.$(
                  '#' + vp_id + '_is_part_of_' + t.meta.virtual_individual_id,
                ).length == 0
              ) {
                $cy.add({
                  group: 'edges',
                  data: {
                    id: vp_id + '_is_part_of_' + t.meta.virtual_individual_id,
                    source: vp_id,
                    target: t.meta.virtual_individual_id,
                    label: 'is_part_of',
                  },
    ...
    $.ajax({
      ...
      success: function(res) {
        ...
        if (res.results.length > 0) {
          ...
          res.results.slice(0, 10).forEach(function(ann) {
            getshit({ query: { url: ann.url } });
            // console.log(ann.url);
          });
        }
        counter -= 1;
