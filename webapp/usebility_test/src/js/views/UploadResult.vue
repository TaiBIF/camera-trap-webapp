            ...
            <div class="form-group row">
              ...
              <div class="col-11">
                <div class="checkbox" v-for="(ca,idx) in camara" :key="`camara-no-${ca}`">
                  <input type="checkbox" class="form-control" :id="`camara-${idx}`" :value="ca" :checked="($route.params.type==='csv' && ca==='HC20A' || $route.params.type==='zip' && ca==='HC20B')">
...
export default {
  ...
  computed: {
    ...mapGetters([
      'table_rows', 'species', 'camara', 'currentRow', 'currentColumn', 'FileType'
    ])
  },
    watch: {
      // 如果路由有变化，会再次执行该方法
  ...
  methods: {
    ...
    fetchData() {
      if(['jpeg','jpg','png'].indexOf(this.$route.params.type)>-1) {
        // alert('single photo')
      } else {
        // alert('package photo');
      }
  ...
  beforeMount() {
    ...
    if(['jpeg','jpg','png'].indexOf(this.$route.params.type)>-1) {
      link = 'for-photo-upload'
    }
    if(this.$route.params.type=='csv') {
      link = 'for-csv-upload'
    } 
    if(this.$route.params.type=='zip') {
      link = 'for-zip-upload'
    }
    axios.get(`../csv/${link}.csv`).then((response) => {
      var data = [];
      var rows = [];
